"""Jarvis Simulation Engine - Predictive scenario simulation and strategy evaluation.

The Simulation Engine enables Jarvis to predict future outcomes before making decisions.
...
World Model Engine
        ↓
  Simulation Engine
  ├─ Scenario Generator → Generate possible futures
  ├─ Strategy Simulator → Execute strategies in scenarios
  ├─ Outcome Predictor → Predict results with confidence
  ├─ Risk Analyzer → Evaluate risks and uncertainties
  ├─ Simulation Memory → Learn from past results
  └─ Core Orchestrator → Coordinate pipeline
        ↓
...
**Input Providers:**
- World Model Engine → World state and context
- Knowledge System → Entity/relationship context
- Reasoning Engine → Strategies to evaluate

**Output Consumers:**
- Reasoning Engine ← Strategy rankings and recommendations
- Memory System ← Simulation results for learning
- Self-Improvement Engine ← Performance metrics

### Features

✓ **Multi-scenario simulation** - Optimistic, realistic, pessimistic, alternative
✓ **Step-by-step execution** - Track bottlenecks and failure points
✓ **Probabilistic outcomes** - Success rates with confidence metrics
✓ **Risk analysis** - Identify critical risks and mitigations
✓ **Comparative evaluation** - Rank strategies by expected value and risk
✓ **Learning from history** - Improve predictions over time
✓ **Adaptive simulation** - Adjust predictions based on past accuracy
✓ **Executive summaries** - Brief recommendations with reasoning
...
# Core components
from jarvis.simulation_engine.simulation_core import (
    SimulationEngine,
    SimulationRequest,
    SimulationResult,
    SimulationStatus,
...
from jarvis.simulation_engine.scenario_generator import (
    ScenarioGenerator,
    Scenario,
    ScenarioType,
)

from jarvis.simulation_engine.outcome_predictor import (
    OutcomePredictor,
    OutcomePrediction,
)

from jarvis.simulation_engine.risk_analyzer import (
    RiskAnalyzer,
    Risk,
)

from jarvis.simulation_engine.strategy_simulator import (
    StrategySimulator,
    StrategyStep,
    SimulationExecution,
)

from jarvis.simulation_engine.simulation_memory import (
    SimulationMemory,
    SimulationRecord,
)
...
__version__ = "1.0.0"
__doc_title__ = "Jarvis Simulation Engine"
