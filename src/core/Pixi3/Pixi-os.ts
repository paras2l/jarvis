...
import { PixiExecutionReport, PixiPlanStep, PixiStepExecutionResult } from './types'
...
class PixiOperatingSystem {
  ...
  async executeGoal(goal: string, options: ExecuteGoalOptions = {}): Promise<PixiExecutionReport> {
    ...
    const results: PixiStepExecutionResult[] = []
    ...
    const status: PixiExecutionReport['status'] = failed === 0
      ? 'completed'
      : completed > 0
        ? 'partial'
        : 'failed'
    ...
    const report: PixiExecutionReport = {
      plan,
      status,
      summary,
      results,
  ...
  private async checkApproval(step: PixiPlanStep, goal: string): Promise<{ approved: boolean; reason?: string }> {
  ...
  private async executeStep(step: PixiPlanStep, goal: string): Promise<{ success: boolean; output: string; data?: Record<string, unknown> }> {
  ...
  private async buildSummary(goal: string, results: PixiStepExecutionResult[]): Promise<string> {
    const brief = results
...
export const PixiOS = new PixiOperatingSystem()
