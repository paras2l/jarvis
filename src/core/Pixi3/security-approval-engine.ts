...
import { PixiApprovalDecision, PixiApprovalRequest } from './types'

type ApprovalHandler = (request: PixiApprovalRequest) => Promise<PixiApprovalDecision> | PixiApprovalDecision
...
class SecurityApprovalEngine {
  ...
  async enforcePolicy(action: string): Promise<{ allowed: boolean; reason: string }> {
    ...
    const decision = await policyGateway.decide({
      requestId: `Pixi3_policy_${Date.now()}`,
      agentId: 'Pixi-os',
      action: 'Pixi3_action',
      command: action,
      source: 'local',
  ...
  async requestApproval(action: string, reason: string, payload?: Record<string, unknown>): Promise<PixiApprovalDecision> {
