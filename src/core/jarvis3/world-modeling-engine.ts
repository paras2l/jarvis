import { JarvisContextModel, JarvisPlan, JarvisWorldModelDecision } from './types'

class WorldModelingEngine {
  simulate(goal: string, plan: JarvisPlan, context: JarvisContextModel): JarvisWorldModelDecision {
    const options: JarvisWorldModelDecision['options'] = []
  ...
  private scoreOption(mode: 'cautious' | 'balanced' | 'aggressive', goal: string, plan: JarvisPlan, context: JarvisContextModel): number {
